...
const CollectionCreateForm: FC<CollectionCreateFormProps> = ({
	...
	const saveContractDetails = () => {
		const data = {
			...contractDetails,
			to_date: contractDetails.to_date.toISOString(),
			from_date: contractDetails.from_date.toISOString(),
			total_contract_value: parseFloat(contractDetails.total_contract_value),
		};
		setSavingContract(0);
		apiCall({
			method: "POST",
			...
			handleResponse: (res) => {
				...
				fetchData();
				console.log(savingContract, savingAttachments);
			},
			handleError: (err) => {
				setSavingContract(-1);
			},
	...
	const uploadfiles = (contract_no: any) => {
		...
		Promise.all(promises).then(() => {
			apiCall({
				method: "PUT",
				url: "/clientcontracts",
				data: { id: contract_no, data: { contract_attachment: files } },
				handleResponse: (res) => {
					console.log(res);
		...
		>
			...
			{page === "contract" ? (
				...
				<>
					...
					>
						...
						<Descriptions.Item label="Contract Type">
							{
								contractType.find((x: any) => x.id === contractDetails.type)
									.value
							}
						</Descriptions.Item>
						...
						<Descriptions.Item label="FM Company">
							{contractDetails.fm_company}
						</Descriptions.Item>
						<Descriptions.Item label="Title">
							{contractDetails.title}
						</Descriptions.Item>
						<Descriptions.Item label="From Date">
							{contractDetails.from_date.format("DD/MM/YYYY")}
						</Descriptions.Item>
						<Descriptions.Item label="To Date">
							{contractDetails.to_date.format("DD/MM/YYYY")}
						</Descriptions.Item>
				...
				<Result
					...
					icon={
						savingAttachments === 1 ||
						(savingAttachments === -1 && savingContract === 1) ? undefined : (
							<LoadingOutlined />
						)
					}
					title={
						savingAttachments === 1 ||
						(savingAttachments === -1 && savingContract === 1)
...
const AddNewContract: FC<props> = ({ fetchData }) => {
